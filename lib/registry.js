...
//      the trust anchor: the founding cast.
//   2. FETCHED  — the live character-packs index, fetched at runtime for
//      freshness. Only trusted if it ships a valid signature (index.json.sig)
//      over the same key. Verified live slugs are UNIONED onto the shipped set
//      (live can only ADD eligibility, never revoke the founding cast). If the
//      live manifest is unsigned or the signature fails, it is ignored and we
...
// Pull the membership slugs out of either manifest shape: our slim bundled
// manifest ({slugs:[...]}) or the full character-packs index ({packs:[{slug}]}).
function slugsOf(json) {
  if (!json || typeof json !== "object") return [];
  if (Array.isArray(json.slugs)) return json.slugs.filter((s) => typeof s === "string");
  if (Array.isArray(json.packs)) return json.packs.map((p) => p && p.slug).filter(Boolean);
  return [];
}
