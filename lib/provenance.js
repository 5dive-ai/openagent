...
// ── The `openagent:` URI scheme (DIVE-1214) ────────────────────────────────
// A registered URI scheme an OpenAgent app handles — the bitcoin:<address>
// analogy. Scanning a card QR deep-links into the app to open/verify/provision/
// chat the agent. Format LOCKS to:  openagent:<multibase-key>[?name=&url=]
//   <multibase-key> = the did:key body with the "did:key:" prefix STRIPPED. The
//     leading "z" already self-describes it as base58btc multibase, so the URI
//     stays vendor-neutral (standard multibase key inside, our scheme as the
//     wrapper). The app reconstructs "did:key:" internally for signature verify.
//   name (persona.name) + url (an endpoint to reach the agent) are OPTIONAL
//     query params — a nicer pre-fetch label / deep-link target.
function multibaseFromDidKey(did) {
  const s = String(did || "");
  return s.startsWith("did:key:") ? s.slice("did:key:".length) : s;
}
function openagentUri(did, opts = {}) {
  const key = multibaseFromDidKey(did);
  if (!key) return null;
  const params = [];
  if (opts.name) params.push("name=" + encodeURIComponent(String(opts.name)));
  if (opts.url) params.push("url=" + encodeURIComponent(String(opts.url)));
  return "openagent:" + key + (params.length ? "?" + params.join("&") : "");
}
