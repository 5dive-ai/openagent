...
function formatError(err) {
  ...
  switch (err.keyword) {
    ...
    case "additionalProperties":
      return `${where}: unknown field '${err.params.additionalProperty}' (not in the v0.1 spec)`;
    case "pattern":
...
function validateFile(file) {
  ...
  if (doc === null || typeof doc !== "object" || Array.isArray(doc)) {
    return { file, ok: false, errors: ["top-level document must be a mapping/object"] };
  }

  const validate = getValidator();
  const ok = validate(doc);
  if (ok) {
    return { file, ok: true, id: doc.id, errors: [] };
  }
  ...
  return { file, ok: false, id: doc.id, errors: unique };
}

module.exports = { validateFile, SCHEMA_PATH };
