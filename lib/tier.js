...
function computeTier(persona, ctx = {}) {
  ...
  const checklist = completenessChecklist(p);
  const completeness = Math.round(
    (checklist.filter((c) => c.present).length / checklist.length) * 100
  );
...
// The labeled surface computeTier's `completeness` percent is the fraction of.
// Exposed so the `doctor` command can turn the opaque number into an
// actionable punch-list ("add face.sprite", "fill written.sample"). Each entry
// is { key, label, field, present } — field is the dotted persona path to set.
const COMPLETENESS_FIELDS = [
  { key: "id", label: "id", field: "id", test: (p) => present(p.id) },
  { key: "name", label: "name", field: "name", test: (p) => present(p.name) },
  { key: "role", label: "role", field: "role", test: (p) => present(p.role) },
  { key: "behavior", label: "behavior", field: "behavior", test: (p) => present(p.behavior) },
  { key: "face-ref", label: "face image", field: "face.ref", test: (p) => present((p.face || {}).ref) },
  { key: "face-anchor", label: "face anchor prompt", field: "face.anchor", test: (p) => present((p.face || {}).anchor) },
  { key: "face-full", label: "full-body render", field: "face.full", test: (p) => present((p.face || {}).full) },
  { key: "face-sprite", label: "sprite sheet", field: "face.sprite", test: (p) => present((p.face || {}).sprite) },
  { key: "voice-base", label: "named TTS voice", field: "voice.audio.base", test: (p) => isNamedVoice(((p.voice || {}).audio || {}).base) },
  { key: "voice-style", label: "voice style notes", field: "voice.audio.style", test: (p) => present(((p.voice || {}).audio || {}).style) },
  { key: "voice-ref", label: "voice-clone reference clip", field: "voice.audio.ref", test: (p) => present(((p.voice || {}).audio || {}).ref) },
  { key: "voice-id", label: "cloned voice id", field: "voice.audio.id", test: (p) => present(((p.voice || {}).audio || {}).id) },
  { key: "written-rules", label: "writing-style rules", field: "voice.written.rules", test: (p) => nonEmptyArr(((p.voice || {}).written || {}).rules) },
  { key: "written-sample", label: "writing sample", field: "voice.written.sample", test: (p) => !isStubSample(((p.voice || {}).written || {}).sample) },
  { key: "posts-about", label: "posts_about topics", field: "posts_about", test: (p) => nonEmptyArr(p.posts_about) },
  { key: "links", label: "links", field: "links", test: (p) => nonEmptyObj(p.links) },
];

/** Labeled completeness checklist: [{ key, label, field, present }]. */
function completenessChecklist(persona) {
  const p = persona || {};
  return COMPLETENESS_FIELDS.map((f) => ({
    key: f.key, label: f.label, field: f.field, present: !!f.test(p),
  }));
}
...
const PROGRESSION = {
  ...
  graded:
    "get conferred Mythical by a trusted signed registry — the path is yet to be seen (raise completeness + collect badges meanwhile)",
