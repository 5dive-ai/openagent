...
const { didKeyFromPublicKey, shortDidKey, fingerprintFromDidKey, openagentUri } = require("./provenance");
...
// QR (DIVE — card v2): the top-right QR encodes the agent's `openagent:` URI
// (openagent:<multibase-key>[?name=&url=], DIVE-1214) so a scan deep-links into
// an OpenAgent app to open/verify/provision/chat the agent — the multibase key
// carries the same verifiable did:key (prefix stripped). Rendered ONLY for SIGNED
// personas (those carrying provenance.created_by.key); unsigned cards degrade clean (no QR).
...
function qrModulesForPersona(persona) {
  ...
  // DIVE-1214: the QR encodes the `openagent:` URI (openagent:<multibase-key>
  // [?name=&url=]) — a registered scheme an OpenAgent app handles (bitcoin:<addr>
  // analogy) so a scan deep-links into the app instead of surfacing a raw did:key.
  // The "did:key:" prefix is stripped; the leading "z" already self-describes the
  // multibase key, and the app reconstructs did:key: internally to verify.
  const links = (persona && persona.links) || {};
  const uri = openagentUri(didKey, {
    name: persona && persona.name,
    url: links.agent_card || links.url,
  });
  if (!uri) return null;
  try {
...
function buildSvg(persona, faceDataUri, tierKey, motion, qrModules, extra) {
  ...
  pips += "</g>";
  // QR (top-right): scan -> deep-link into an OpenAgent app via the `openagent:`
  // URI (carries the agent's verifiable multibase key). Rendered only when the
  // caller supplies QR modules (generated from the signed persona's openagent: URI).
