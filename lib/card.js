...
function buildSvg(persona, faceDataUri, tierKey, motion, qrModules) {
  ...
  // Ambient breath: a gentle whole-loop swell on the accent top-glow, on EVERY
  // tier (incl. Common, which has no frame glow rings). One seamless cycle per
  // loop (phase 0 == phase 1). Pairs with the live waveform as the universal motion.
  const ambient = anim ? 0.86 + 0.14 * Math.sin(TAU * phase) : 1;
  ...
  const kb = anim ? 1 + 0.02 * (0.5 - 0.5 * Math.cos(TAU * phase)) : 1;
  ...
  for (let i = 0; i < bars; i++) {
    ...
    const wob = anim ? 0.80 + 0.20 * Math.sin(TAU * (phase * 2 + off)) : 1;
