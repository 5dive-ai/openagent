...
function buildSvg(persona, faceDataUri, tierKey, motion, qrModules) {
  ...
  if (T.foil) {
    ...
    if (anim) {
      const span = W + 600;
      const cx = (-300 + phase * span).toFixed(1);
      foilLayer += `<rect x="${cx}" y="-200" width="380" height="${H + 400}" fill="url(#sheen)" transform="rotate(18 ${cx} ${H / 2})" opacity="${T.holo ? 0.30 : 0.26}"/>`;
    } else if (smil) {
      ...
      foilLayer += `<g transform="rotate(16 ${W / 2} ${H / 2})"><rect y="-320" width="300" height="${H + 640}" fill="url(#sheen)" opacity="0">`
        + `<animate attributeName="x" values="-420;${W + 220}" dur="4.5s" repeatCount="indefinite"/>`
        + `<animate attributeName="opacity" values="0;${peak};${peak};0" keyTimes="0;0.12;0.88;1" dur="4.5s" repeatCount="indefinite"/>`
        + `</rect></g>`;
    }
    ...
    foilLayer += (smil ? `<g>${spk}<animate attributeName="opacity" values="0.6;1;0.75;1;0.6" dur="3.4s" repeatCount="indefinite"/></g>` : spk) + `</g>`;
  }
  if (T.holo) {
