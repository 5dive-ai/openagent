...
function encodeApng(frames, opts = {}) {
  ...
  // Run-length dedup: APNG has no inter-frame delta, so each frame re-encodes the
  // full image (a photo avatar × N frames = multi-MB). With the calm cadence the
  // long REST is byte-identical frame-to-frame, so we collapse each run of
  // identical CONSECUTIVE frames into ONE frame whose on-screen delay = base delay
  // × run length. A 12s loop that's a 3s pass + 9s static rest drops from ~60 full
  // frames to ~(pass frames)+1. Fully back-compatible: when every frame differs
  // (e.g. a continuous pulse) each run is length 1 and the output is byte-identical
  // to the un-deduped encoding.
  const runs = [];
  for (const f of frames) {
    const last = runs[runs.length - 1];
    if (last && last.buf.equals(f)) last.count++;
    else runs.push({ buf: f, count: 1 });
  }

  const first = parseChunks(runs[0].buf);
  ...
  // acTL — animation control. num_frames counts RUNS, not source frames.
  const acTL = Buffer.alloc(8);
  acTL.writeUInt32BE(runs.length, 0);
  ...
  const fcTL = (w, h, dNum) => {
    ...
    // delay = base × run length (PNG delay is a num/den fraction of a second);
    // clamp to the uint16 field max (den is unchanged, so this caps a single run
    // at ~65535/den seconds — far beyond any real loop).
    d.writeUInt16BE(Math.min(dNum, 0xffff), 20);
  ...
  for (let r = 0; r < runs.length; r++) {
    const { buf, count } = runs[r];
    const chunks = r === 0 ? first : parseChunks(buf);
    const idats = chunks.filter((c) => c.type === "IDAT").map((c) => c.data);
    if (idats.length === 0) throw new Error(`run ${r} has no IDAT`);
    parts.push(chunk("fcTL", fcTL(width, height, delayNum * count)));
    if (r === 0) {
