...
function buildReceipt({ taskHash, resultHash, fromDid, toDid, at, title = null }) {
  ...
  // Optional short human-readable label, signed as part of the canonical body so a
  // feed can display it without trusting any unsigned, forgeable caption. task/
  // result stay content hashes; `title` is a deliberate, signed disclosure. Added
  // only when present → receipts without it build byte-identically (back-compat).
...
function verifyHistory(lines, selfDid) {
  ...
  const seen = new Set(); // canonical-body hashes — a replayed receipt counts once
  const bad = [];
  for (const line of lines) {
    ...
    // Dedup: an identical receipt (same canonical body) counts ONCE, so a
    // relayed/replayed receipt can't pad the ledger. (Pre-public hardening: a
    // per-receipt body nonce makes even same-content interactions distinct.)
